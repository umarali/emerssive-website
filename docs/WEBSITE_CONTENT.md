# Website Content Reference

Complete reference of all visible content on the Emerssive website. Use this document when making content updates or understanding the site's messaging.

---

## Company Information

| Field | Value |
|-------|-------|
| **Company Name** | Emerssive Solutions |
| **Brand Name** | Emerssive |
| **Email** | contact@emerssive.com |
| **Phone** | +923314739747 |
| **Address** | 124L, Johar Town, Lahore. 54700 |
| **Booking Link** | https://calendar.app.google/U3z2GqSza9Fhch8e9 |
| **LinkedIn** | https://pk.linkedin.com/company/emerssive |
| **Facebook** | https://www.facebook.com/emerssive |

---

## Navigation

**Menu Items:**
- Services → #services
- Case Studies → #case-studies

**CTA Button:** "Book a Call" → Google Calendar booking link

---

## Hero Section

**Pre-heading Badge:** "Welcome to Emerssive"

**Main Headline:**
```
We build
[rotating phrase]
that ship in weeks
```

**Rotating Phrases** (2.5s interval):
1. AI agents
2. AI powered apps
3. automations
4. web apps
5. mobile apps

**Subheadline:**
> We help innovative companies work smarter with custom AI solutions and software development. From generative AI to web and mobile technologies — we build software that drives your success.

**CTAs:**
1. "Book a Free Consultation →" (primary)
2. "See case studies" (secondary)

**Trust Indicator:**
> Trusted by 100+ happy clients including affiliates and acquisitions of Fortune 500 companies

**KPI Card:**
- **91%** projects delivered on time and budget

---

## Services Section

**Section Title:** "What we can do for you"

**Section Subtitle:** "We help entrepreneurs and business leaders build and launch innovative software solutions."

### Service 1: Generative AI
- **Title:** Generative AI
- **Description:** Falling behind in the AI race? We help you move from confusion to execution. Discover what GenAI can do for your business.
- **Image:** `/assets/services/gen_ai_service.svg`

### Service 2: Web/Mobile Development
- **Title:** Web/Mobile Development
- **Description:** Need a site that's fast, secure, and built to convert? We handle design, code, and performance end-to-end. Let's bring your website to life.
- **Image:** `/assets/services/web_mobile_service.svg`

---

## Case Studies Section

**Section Label:** "Our Success Stories"

**Section Title:** "Delivering exceptional results through innovative development solutions"

### Case Study 1: Fluxx
| Field | Value |
|-------|-------|
| Company | Fluxx |
| Subtitle | Grant Management Platform |
| Logo | `/assets/customers/fluxx-removebg-preview.png` |
| Key Metric | **35%** |
| Metric Label | reduction in manual errors |
| Description | Enhanced grant management platform with advanced system architecture and financial integrations. Automated financial integration eliminated manual processes and saved 500+ hours annually. |
| Notable Clients | Citi Foundation, USAID, Leukemia & Lymphoma Society |

### Case Study 2: Qodex.ai
| Field | Value |
|-------|-------|
| Company | Qodex.ai |
| Subtitle | API Testing & Security Platform |
| Logo | `/assets/customers/qodexai-removebg-preview.png` |
| Key Metric | **78K+** |
| Metric Label | APIs secured globally |
| Description | Built complete AI-powered API testing platform from concept to enterprise-scale solution. Enables 80% faster testing with natural language processing for development teams. |

### Case Study 3: BookBiteAI
| Field | Value |
|-------|-------|
| Company | BookBiteAI |
| Subtitle | Restaurant Management System |
| Logo | `/assets/customers/bookbite-removebg-preview.png` |
| Key Metric | **30s** |
| Metric Label | average reservation time |
| Description | Built an AI-powered restaurant management platform that streamlines operations and enhances customer experience. |

### Case Study 4: Frontline Education
| Field | Value |
|-------|-------|
| Company | Frontline Education |
| Subtitle | Educational Technology Platform |
| Logo | `/assets/customers/frontline-education-removebg-preview.png` |
| Key Metric | **1.2M+** |
| Metric Label | monthly visitors supported |
| Description | Updated their outdated design with a new website that supports 1.2M+ visitors and provides enhanced user experience for educational institutions. |
| Notable Clients | Schools, Districts, Educational Institutions |

---

## Customer Logos Section

**Section Title:** "Trusted by Industry Leaders"

**Section Description:** "We've partnered with innovative companies across various industries to deliver cutting-edge solutions that drive growth and success."

### Customers (8)
| Company | Logo Path |
|---------|-----------|
| Wazer | `/assets/customers/wazer-removebg-preview.png` |
| PuffBar | `/assets/customers/puffbar-removebg-preview.png` |
| Frontline Education | `/assets/customers/frontline-education-removebg-preview.png` |
| Lendica | `/assets/customers/lendica-removebg-preview.png` |
| Fluxx | `/assets/customers/fluxx-removebg-preview.png` |
| QodexAI | `/assets/customers/qodexai-removebg-preview.png` |
| InRule | `/assets/customers/inrule-removebg-preview.png` |
| AppSeed | `/assets/customers/appseed-removebg-preview.png` |

---

## Testimonials Section

**Section Title:** "Success, Powered by Emerssive"

**Section Subtitle:** "Discover how we've helped businesses achieve their goals through innovative solutions and dedicated partnership."

### Testimonial 1: Jerry Shu
| Field | Value |
|-------|-------|
| Name | Jerry Shu |
| Role | Co-Founder |
| Company | Lendica |
| Image | `/assets/testimonials/Jerry Shu - Co-Founder - Lendica.png` |
| Quote | Working with Umar was an extremely pleasant experience as he was extremely communicative and flexible, and was able to complete our task of setting up a CI/CD pipeline faster than our expected timeline. Would highly recommend his services to anyone in need! |

### Testimonial 2: Adi Chirilov
| Field | Value |
|-------|-------|
| Name | Adi Chirilov |
| Role | CTO |
| Company | AppSeed |
| Image | `/assets/testimonials/Adi Chirilov - CTO - AppSeed.png` |
| Quote | Umar helped our team with many projects, from average to high complexity. Besides his excellent technical skills, Umar is a good communicator and a proactive team member, positively influencing the whole team's performance. Highly recommended for React, Javascript tasks, and Tech Lead jobs |

### Testimonial 3: Lindsey Holmes
| Field | Value |
|-------|-------|
| Name | Lindsey Holmes |
| Role | Chief of Product |
| Company | Code3Apps |
| Image | `/assets/testimonials/Lindsey Holmes - Chief of Product - Code3Apps.png` |
| Quote | Umar and Code3Apps worked together for a couple years and Umar demonstrated a great work ethic, good technical knowledge (our project was a web based platform with an app interface), and was overall a pleasure to interact with. |

### Testimonial 4: Justus Luttig
| Field | Value |
|-------|-------|
| Name | Justus Luttig |
| Role | Investment Associate |
| Company | Authentic Ventures |
| Image | `/assets/testimonials/Justus Luttig - Investment Associate - Authentic Ventures.png` |
| Quote | We hired Umar to help with a complex problem on our website back end. He has a great understanding and proficiency in Vue, Node.js, MongoDB, and Heroku. Highly recommend Umar! |

---

## FAQ Section

**Section Title:** "Frequently Asked Questions"

### Q1: What industries do you cater to?
> We cater to various industries such as finance, education, e-commerce, real estate, hospitality, logistics, and startups.

### Q2: How do you manage projects?
> We follow agile methodologies with regular updates and iterations to ensure transparency and flexibility throughout the development process.

### Q3: What technologies do you specialize in?
> Our expertise includes Python/Django, JavaScript/React, React Native, Docker/Kubernetes, LangChain, and Shopify.

### Q4: How do you ensure quality?
> We implement thorough testing phases including manual and automated QA to ensure bug-free and high-quality software.

### Q5: What services do you offer?
> We specialize in Generative AI, Mobile and Web Development, and AI-powered Automation solutions to drive innovation and efficiency for your business.

### Q6: What scale of projects do you work on?
> The minimum project cost we accept starts at $2,000.

---

## Footer

**Company Description:**
> Emerssive is a young, full-service software development company specializing in Generative AI, Web and Mobile. Our experienced team transforms concepts into impactful applications for startups to enterprise companies.

**Footer Links:**
- Services → #services
- Case Studies → #case-studies

**Contact:**
- Phone: +923314739747
- Email: contact@emerssive.com
- Address: 124L, Johar Town, Lahore. 54700.

**Social:**
- LinkedIn: https://pk.linkedin.com/company/emerssive
- Facebook: https://www.facebook.com/emerssive

**Copyright:** Copyright © 2025 Emerssive Solutions | All Rights Reserved

---

## Sticky Bottom CTA

**Profile Image:** `/assets/Umar.png`

**Message:** "Hey, I'm Umar from Emerssive Solutions. Let's find the right solution to make your team work smarter."

**Button:** "Get in Touch" → Google Calendar booking link

---

## SEO Metadata

| Field | Value |
|-------|-------|
| Title | Emerssive - A full-service software development company |
| Description | Emerssive is a full-service software development company specializing in Generative AI, Web and Mobile. Our experienced team transforms concepts into impactful applications for startups to enterprise companies. |
| Keywords | software development, generative ai, web development, mobile development, startups, enterprise companies |
| Author | Emerssive |
| OG Type | website |
